...
import { db } from '../Components/firebase'; // You'll need to create this file
...
const Payment = () => {
  ...
  // Generate unique ID for new payment methods
  ...
  const fetchPaymentMethods = async () => {
    try {
      ...
      if (userDoc.exists()) {
        ...
        // Sort payment methods by createdAt in descending order
  ...
  const createPaymentMethod = async (paymentData) => {
    try {
      ...
      let currentMethods = [];
      if (userDoc.exists()) {
        currentMethods = userDoc.data().paymentMethods || [];
      }
  ...
  const updatePaymentMethod = async (paymentId, paymentData) => {
    try {
      ...
      let currentMethods = userDoc.data().paymentMethods || [];
      ...
      currentMethods = currentMethods.map(method => {
        if (method.id === paymentId) {
          return {
            ...method,
            ...paymentData,
            id: paymentId, // Preserve the original ID
            updatedAt: new Date()
          };
        }
        return method;
